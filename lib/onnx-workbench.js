...
const { buildCropTarget, buildTextExportContext } = require('./text-export-context');
const {
...
function parseTypeProto(typeProto) {
    ...
    const dynamicDims = Array.isArray(shape)
        ? shape.map((dimension, index) => isStaticDimension(dimension) ? -1 : index).filter((index) => index >= 0)
        : [];
    return {
        kind: 'tensor',
        dataType,
...
function toTypeFromOrtMetadata(entry) {
    ...
    return {
        ...
        dynamicDims: Array.isArray(shape) ? shape.map((dimension, index) => isStaticDimension(dimension) ? -1 : index).filter((index) => index >= 0) : [],
        optional: false
    };
}
...
function normalizeConstantTensorName(tensor, outputName) {
    const clone = cloneProto(onnx.TensorProto, tensor);
    clone.name = outputName;
    return clone;
}

function createScalarTensor(outputName, dataType, fieldName, value) {
...
function createVectorTensor(outputName, dataType, fieldName, values) {
    const items = ensureArray(values);
    return onnx.TensorProto.create({
        name: outputName,
        dataType,
        dims: [items.length],
        [fieldName]: items.slice()
    });
}

function constantTensorFromAttribute(attribute, outputName) {
    if (!attribute || !outputName) {
        return null;
    }
    switch (attribute.name) {
        case 'value':
            return attribute.t ? normalizeConstantTensorName(attribute.t, outputName) : null;
        case 'value_float':
            return attribute.f !== null && attribute.f !== undefined
                ? createScalarTensor(outputName, DATA_TYPE.FLOAT, 'floatData', Number(attribute.f))
                : null;
        case 'value_floats':
            return ensureArray(attribute.floats).length > 0
                ? createVectorTensor(outputName, DATA_TYPE.FLOAT, 'floatData', ensureArray(attribute.floats).map((value) => Number(value)))
                : null;
        case 'value_int':
            return attribute.i !== null && attribute.i !== undefined
                ? createScalarTensor(outputName, DATA_TYPE.INT64, 'int64Data', attribute.i)
                : null;
        case 'value_ints':
            return ensureArray(attribute.ints).length > 0
                ? createVectorTensor(outputName, DATA_TYPE.INT64, 'int64Data', ensureArray(attribute.ints))
                : null;
        case 'value_string':
            return attribute.s && attribute.s.length > 0
                ? createScalarTensor(outputName, DATA_TYPE.STRING, 'stringData', Buffer.from(attribute.s))
                : null;
        case 'value_strings':
            return ensureArray(attribute.strings).length > 0
                ? createVectorTensor(outputName, DATA_TYPE.STRING, 'stringData', ensureArray(attribute.strings).map((value) => Buffer.from(value)))
                : null;
        default:
            return null;
    }
...
function sparseConstantFromAttribute(attribute, outputName) {
    if (!attribute || attribute.name !== 'sparse_value' || !attribute.sparseTensor || !outputName) {
        return null;
    }
    const clone = cloneProto(onnx.SparseTensorProto, attribute.sparseTensor);
    if (clone.values) {
        clone.values.name = outputName;
    }
    return clone;
}
...
class ONNXWorkbench {
    ...
    _analyzeGraph(model, modelDir) {
        ...
        for (const node of nodes) {
            if (node.opType !== 'Constant' || node.outputs.length !== 1) {
                continue;
            }
            const outputName = node.outputs[0];
            const value = ensureValue(outputName);
            if (value.initializer) {
                continue;
            }
            for (const attribute of ensureArray(node.raw.attribute)) {
                const sparse = sparseConstantFromAttribute(attribute, outputName);
                if (sparse) {
                    value.initializer = sparse;
                    value.type = value.type || inferTensorTypeFromInitializer(sparse.values || sparse);
                    initializers.set(outputName, sparse);
                    break;
                }
                const tensor = constantTensorFromAttribute(attribute, outputName);
                if (tensor) {
                    value.initializer = tensor;
                    value.type = value.type || inferTensorTypeFromInitializer(tensor);
                    initializers.set(outputName, tensor);
                    break;
                }
            }
        }
    ...
    hasExternalData(session) {
        if (!session || !session.graphInfo || !session.graphInfo.initializers) {
            return false;
        }
        return Array.from(session.graphInfo.initializers.values()).some((tensor) => tensor.dataLocation === DATA_LOCATION.EXTERNAL && ensureArray(tensor.externalData).length > 0);
    }
    ...
    buildTextExportContext(artifactId) {
        ...
        return buildTextExportContext(session, artifact);
    }
    ...
    getCropTarget(artifactId) {
        ...
        const session = this.getSession(artifact.modelSessionId);
        return buildCropTarget(session, artifact);
    }
            ...
            info.type = onnx.TypeProto.create({
                tensorType: onnx.TypeProto.Tensor.create({
                    ...
                    shape: onnx.TensorShapeProto.create({
                        dim: ensureArray(value.type.shape).map((dimension) => isStaticDimension(dimension)
                            ? onnx.TensorShapeProto.Dimension.create({ dimValue: dimension })
                            : onnx.TensorShapeProto.Dimension.create({ dimParam: dimension ? String(dimension) : '?' }))
                    })
                })
            });
        }
    ...
    resolveImportedInput(token) {
        return this._resolveImportedInput(token);
    }
    ...
    assignCompareSlot(slot, artifactId) {
        if (slot === 'A') {
            this.compareState.slotA = this.getCompareSlot(artifactId);
        } else if (slot === 'B') {
            this.compareState.slotB = this.getCompareSlot(artifactId);
        } else {
    ...
    getCompareSlot(artifactId) {
        ...
        return this._toCompareSlot(artifact);
    }
    ...
    _recomputeBindings() {
        ...
        this.compareState.inputBindings = buildCompareBindings(slotA.ioSignature.inputs, slotB.ioSignature.inputs);
        this.compareState.outputBindings = buildCompareBindings(slotA.ioSignature.outputs, slotB.ioSignature.outputs);
    }
    ...
    _resolveCompareBindings() {
        return resolveCompareBindings(this.compareState);
    }
    ...
    async runCompare(options = {}) {
        ...
        try {
            ...
            const imported = this._resolveImportedInput(options.importToken || (this.compareState.importedInput && this.compareState.importedInput.token));
            const sharedFeeds = {};
            const sharedInputPlan = buildSharedInputPlan(inputBindings, slotB, inputShapes);
            for (const input of sharedInputPlan) {
                const prepared = this._generateInputData({ name: input.sourceName, dtype: input.dtype, shape: input.shape }, options.inputMode || 'zeros', imported, [input.sourceName, input.targetName]);
                sharedFeeds[input.sourceName] = prepared;
            }

            this.compareState.compareRunStatus = { status: 'running', stage: '执行 A', message: '', updatedAt: nowIso() };
            this._emitChange();
            const resultA = await this.runCompareArtifact({ artifactId: slotA.artifactId, sharedFeeds, inputBindings, side: 'A' });
            this.compareState.compareRunStatus = { status: 'running', stage: '执行 B', message: '', updatedAt: nowIso() };
            this._emitChange();
            const resultB = await this.runCompareArtifact({ artifactId: slotB.artifactId, sharedFeeds, inputBindings, side: 'B' });
            ...
            const rows = buildCompareRows(outputBindings, resultA.outputsSummary, resultB.outputsSummary);
    ...
    async runCompareArtifact(options = {}) {
        return this._runCompareSide(options.artifactId, options.sharedFeeds || {}, options.inputBindings || [], options.side || 'A');
    }

    async _runCompareSide(artifactId, sharedFeeds, inputBindings, side = 'A') {
        const artifact = this.getArtifact(artifactId);
        const session = this.getSession(artifact.modelSessionId);
