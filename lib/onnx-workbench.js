...
class ONNXWorkbench {
    ...
    async _inferArtifactEndpointTypes(artifactLike) {
        try {
            const model = this._createArtifactModel(artifactLike, { inlineWeights: true });
            ...
            for (const entry of ensureArray(ortSession.inputMetadata)) {
                if (entry && entry.name) {
                    inputTypes.set(entry.name, toTypeFromOrtMetadata(entry));
                }
            }
            for (const entry of ensureArray(ortSession.outputMetadata)) {
                if (entry && entry.name) {
                    outputTypes.set(entry.name, toTypeFromOrtMetadata(entry));
                }
            }
