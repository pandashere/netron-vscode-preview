...
function toPublicEntry(entry) {
    return {
        ...
        userInputs: Array.isArray(entry.userInputs) ? entry.userInputs.map((item) => ({ ...item })) : [],
        status: entry.status,
        reason: entry.reason || '',
...
function normalizeUserInputs(value, details) {
    if (value === undefined) {
        return [];
    }
    if (!Array.isArray(value)) {
        details.push('Field userInputs must be an array.');
        return [];
    }
    if (value.length > 3) {
        details.push('Field userInputs can contain at most 3 items.');
    }
    const result = [];
    value.slice(0, 3).forEach((item, index) => {
        if (!item || typeof item !== 'object' || Array.isArray(item)) {
            details.push(`Field userInputs[${index}] must be an object.`);
            return;
        }
        ...
        if (!id) {
            details.push(`Field userInputs[${index}].id is required.`);
            return;
        }
        if (!/^[A-Za-z_][A-Za-z0-9_-]*$/.test(id)) {
            details.push(`Field userInputs[${index}].id must start with a letter or underscore and contain only letters, numbers, underscores, or hyphens.`);
        }
        if (!label) {
            details.push(`Field userInputs[${index}].label is required.`);
        }
        result.push({
            id,
            label,
    ...
    for (const item of result) {
        if (ids.has(item.id)) {
            details.push(`Duplicate user input id: ${item.id}`);
        }
        ids.add(item.id);
    }
    return result;
...
function validateManifest(raw, manifestPath, options = {}) {
    ...
    const userInputs = normalizeUserInputs(raw.userInputs, details);
    if (!id) {
